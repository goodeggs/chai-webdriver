...
- `expect(selector).dom.to.have.attribute('attribute', value)` - Test an element's `attribute` versus `value`. `value` can be a `RegExp` or a `String`. By omitting `value` test simply checks for existance of attribute.
